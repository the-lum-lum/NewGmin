...
def compute_damage_with_memory_network(
    ...
    *,
    ...
    lambda_net: float = 1e-3,       # ← stronger coupling
    epsilon_cross: float = 0.4,
    seed_contrast: bool = False,    # ← new flag
    iteration=None,
):
    """
    Network-coupled degradation / recovery with:
      • ≥k-step memory condition
      • bidirectional diffusion (no sign check on diff)
      • optional ±5 % random contrast on first call
    """
    B_min=1e-3
    n_fil, n_seg = angles.shape
    rng = np.random.default_rng()

    # ───────── optional initial heterogeneity ──────────
    #if seed_contrast and iteration == 1:
    #    contrast = rng.uniform(0.95, 1.05, size=prev_stiffness.shape)
    #prev_stiffness = np.clip(prev_stiffness * contrast, 1e-3, B_max)

    # ───────── ρ-matrix (local recovery weight) ─────────
    rho = np.zeros_like(prev_stiffness)
    calm = memory_matrix >= k                      # *** changed condition ***
    rho[calm] = beta * (B_max - prev_stiffness[calm]) * \
                np.exp(-np.abs(angles[calm]) / theta_rec)

    new_B = np.empty_like(prev_stiffness)
    spike_mask = np.zeros_like(prev_stiffness, bool)
    ...
    for i in range(n_fil):
        for j in range(n_seg):
            ...
            # random spike
            if rng.random() < 0.01:
                D_loc = np.clip(D_loc + 0.2, 0.0, 1.0)
                spike_mask[i, j] = True
            ...
            for di in (-1, 0, 1):
                for dj in (-1, 0, 1):
                    ...
                    if 0 <= ni < n_fil and 0 <= nj < n_seg:
                        ...
                        if diff > 0.0:                     # ← **only stiff→soft flow**
            ...
            new_B[i, j] = np.clip(B_tmp - delta + rec_local + rec_net, 1e-3, B_max)

    print(f"[damage] random spikes: {spike_mask.sum()}")
