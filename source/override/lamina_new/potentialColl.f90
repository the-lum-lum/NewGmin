MODULE potential
  ...
  DOUBLE PRECISION, ALLOCATABLE :: BE(:)
  INTEGER, ALLOCATABLE :: BONDS(:,:), TRIANGLES(:,:)
  DOUBLE PRECISION :: F_I, F_S, L, H, K1, K2, K3, R1, R2, R3, BI, LS, HS, CM1, KC, NORM_ANG, KY, WY,LOWER_RAT
  DOUBLE PRECISION :: KAREA, CAREA, AREA0
  INTEGER :: N_ROD, N_SEG, i, j, k, N_BONDS, IO, COLLAGEN_LEN, COLLAGEN_STAGGER
...
SUBROUTINE INIT_SYSTEM()
  ...
  INTEGER :: CUR, CURINDEX, CURBEAM, NODE1, NODE2, NODE0, S1
  

  
  ALLOCATE(COORDS(N_ROD*(N_SEG+1),2), R_ARRAY(3), COORDS_0(N_ROD,2), ANGLES(N), GRAD(N), GRAD_SPAT(N_ROD*(N_SEG+1),2))
  COORDS=0.0
  ANGLES=0.0
  GRAD=0.0
  COORDS_0=0.0
...
SUBROUTINE COMPUTE_ENERGY(E)
  ...
  INTEGER :: CUR, CURINDEX, CURBEAM, NODE1, NODE2
  INTEGER :: BONDTYPE, CURP, CURM, CUR2P, CUR2M, CUR2
  ...
  DO j=2,N_SEG-1
	CUR=j
	EB = EB + (2.0*BE(j)/LS)*( tan(0.5*(ANGLES(CUR)-ANGLES(CUR-1))) )**2
  ENDDO
  ...
  DO i=1,N_BONDS
	...
	!With 1/R term
  CURINDEX=MOD(i-1,N_SEG)+1
  CURBEAM=(i-CURINDEX)/N_SEG
	!print *, i, CURINDEX,CURBEAM
	!ES = ES + K_ARRAY(BONDTYPE,CURBEAM,CURINDEX)*(R-R_ARRAY(BONDTYPE))**2*(1.0/R)
	!Without 1/R term
	ES = ES + K_ARRAY(BONDTYPE,CURBEAM,CURINDEX)*(R-R_ARRAY(BONDTYPE))**2
  ENDDO
  ES = 0.5*ES
  ...
  DO i=1,N_ROD
  CUR=(i-1)*(N_SEG-1)+1
  EC = EC + KC*(1-COS(ANGLES(CUR)))
  ENDDO

  DO i=1,N_ROD
  CUR=(i-1)*(N_SEG-2)+N_SEG-1
  EC = EC + KC*(1-COS(ANGLES(CUR)))
  ENDDO
...
SUBROUTINE COMPUTE_GRAD()
  ...
  INTEGER :: CUR, CURP, CURM, CURBEAM, CURINDEX, NODE1, NODE2, BONDTYPE, NODE_J
  INTEGER :: NODE_K, END_NODE_K, CUR2, CUR2P, CUR2M, CUR2PP, CUR2MM, GSIGN
 ...
 !1) Bending gradient
  ...
  DO i=2,N_ROD-1
	DO j=2,N_SEG-2
		...
		GRAD(CUR) = GRAD(CUR) + (2.0/LS)*(B_ARRAY(i,j)*TAN(THETAM)/(COS(THETAM))**2 - B_ARRAY(i,j+1)*TAN(THETAP)/(COS(THETAP))**2 )
	ENDDO 
  ENDDO
  ...
  DO i=2,N_ROD-1
	j=1
		...
		GRAD(CUR) = GRAD(CUR) + (2.0*B_ARRAY(i,2)/LS)*( - TAN(THETAP)/(COS(THETAP))**2 )
  ENDDO
  ...
  DO i=2,N_ROD-1
	j=N_SEG-1
		...
		GRAD(CUR) = GRAD(CUR) + (2.0*B_ARRAY(i,j)/LS)*( TAN(THETAM)/(COS(THETAM))**2 )
  ENDDO
  ...
  i=1
	DO j=2,N_SEG-2
		...
		GRAD(CUR) = GRAD(CUR) + (BE(j)*2/LS)*TAN(THETAM)/(COS(THETAM))**2 - (2*BE(j+1)/LS)*TAN(THETAP)/(COS(THETAP))**2 
	ENDDO 
  ...
  i= N_ROD
	DO j=2,N_SEG-2
		...
		GRAD(CUR) = GRAD(CUR) + (2*BE(j)/LS)*TAN(THETAM)/(COS(THETAM))**2 - (2*BE(j+1)/LS)*TAN(THETAP)/(COS(THETAP))**2 
	ENDDO 
  ...
  i=1
	j=1
		...
		GRAD(CUR) = GRAD(CUR) + (2*BE(2)/LS)*( - TAN(THETAP)/(COS(THETAP))**2 )
  ...
  i=1
	j=N_SEG-1
		...
		GRAD(CUR) = GRAD(CUR) + (2*BE(j)/LS)*( TAN(THETAM)/(COS(THETAM))**2 ) 
  ...
  CURINDEX=MOD(i-1,N_SEG)+1
  CURBEAM=(i-CURINDEX)/N_SEG
	!TEMP=K_ARRAY(BONDTYPE,CURBEAM,CURINDEX)*(COORDS(NODE1,1)-COORDS(NODE2,1))*(1.0-R_ARRAY(BONDTYPE)/R)*(1/R*(1-0.5*(1.0-R_ARRAY(BONDTYPE)/R)))
	!Without 1/R term
	TEMP=K_ARRAY(BONDTYPE,CURBEAM,CURINDEX)*(COORDS(NODE1,1)-COORDS(NODE2,1))*(1.0-R_ARRAY(BONDTYPE)/R)
	...
	! Y-derivatives
	!With 1/R term
	!TEMP=K_ARRAY(BONDTYPE,CURBEAM,CURINDEX)*(COORDS(NODE1,2)-COORDS(NODE2,2))*(1.0-R_ARRAY(BONDTYPE)/R)*(1/R*(1-0.5*(1.0-R_ARRAY(BONDTYPE)/R)))
	!Without 1/R term
	TEMP=K_ARRAY(BONDTYPE,CURBEAM,CURINDEX)*(COORDS(NODE1,2)-COORDS(NODE2,2))*(1.0-R_ARRAY(BONDTYPE)/R)
...
!5)Angular constraint
  ...
  DO i=1,N_ROD
   CUR=(i-1)*(N_SEG-1)+1
   GRAD(CUR) =  GRAD(CUR)+SIN(ANGLES(CUR))
  ENDDO
